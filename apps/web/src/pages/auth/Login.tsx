...
import { fetcher, saveAuthTokens, saveCurrentUser } from '../../lib/api';
...
const Login: React.FC = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      saveCurrentUser({ id: data.user.id, nickname: data.user.nickname });
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.footerSection}>
        ...
        <span className={styles.footerTextBlack} onClick={() => navigate('/signup')}>
          회원가입
        </span>
      </div>
      ...
      {showModal && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="flex w-full max-w-[340px] flex-col items-center rounded-2xl bg-white p-6 shadow-xl">
            ...
            <h3 className="mb-2 text-center text-lg font-bold text-neutral-900">로그인 실패</h3>
            <p className="mb-6 whitespace-nowrap text-center text-sm font-medium leading-relaxed text-neutral-500">
              {modalMessage}
            </p>
            <button
              onClick={closeModal}
