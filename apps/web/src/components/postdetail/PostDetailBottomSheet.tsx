...
function extractStructuredFeedback(data: GetRankingPostDetailResponse | null) {
  ...
  const parsedRows = feedbackSummary
    ...
    .filter(
      (
        item,
      ): item is { label: string; count: number; side: "LIKE" | "DISLIKE" } => Boolean(item),
    );
...
export function RankingDetailSheetContent({
  ...
  const explicitPeriod: RankingPeriod | null =
    period ??
    (periodParam === "WEEKLY" || periodParam === "MONTHLY" ? periodParam : null);
  ...
  const likePercent = useMemo(
    () => (totalCount <= 0 ? 0 : Math.round((likeCount / totalCount) * 100)),
    [likeCount, totalCount],
  );
  const dislikePercent = useMemo(
    () => (totalCount <= 0 ? 0 : 100 - likePercent),
  ...
  const outfitItems = useMemo(
    () => (Array.isArray(postData?.outfitItems) ? postData.outfitItems : []),
    [postData],
  );
  ...
  return (
    <div className={detailStyles.sheetContent}>
      <div className={detailStyles.sheetHeader}>
        <div className={detailStyles.sheetHeaderCopy}>
          <div className={detailStyles.authorMetaRow}>
            <p className={detailStyles.authorName}>{postData.author?.nickname ?? "닉네임"}</p>
          </div>
          <p className={detailStyles.contentText}>{postData.content || "코디 설명이 없습니다."}</p>
        </div>
      ...
      <div className={detailStyles.keywordLaneSection}>
        <div className={detailStyles.keywordLaneRow}>
          ...
          {!hideFeedLink ? (
            <button
              type="button"
              className={detailStyles.feedLinkButton}
      ...
      <div className={detailStyles.sectionHeaderRow}>
        ...
        <span className={detailStyles.sectionMetaText}>{formatCount(totalCount)}명 참여</span>
      </div>
      ...
      <div className={detailStyles.evaluationTrack}>
        <div className={detailStyles.evaluationLikeFill} style={{ width: `${likePercent}%` }} />
        <div
      ...
      <div className={detailStyles.sectionDivider} />
      <div className={detailStyles.outfitHeaderRow}>
        <h3 className={detailStyles.outfitTitle}>착용 아이템</h3>
      </div>

      <div className={detailStyles.itemScroll}>
        {outfitItems.length > 0 ? (
          outfitItems.map((item, index) => (
            <div key={item.id ?? index} className={detailStyles.outfitCard}>
              <div className={`${detailStyles.outfitField} ${detailStyles.outfitCategoryField}`}>
                <div className={detailStyles.outfitCategoryInner}>
                  <Tag size={13} strokeWidth={2} className={detailStyles.outfitCategoryIcon} />
                  <span className={detailStyles.outfitFieldValue}>
                    {formatCategoryLabel(item.category)}
                  </span>
                </div>
              </div>
              <div className={detailStyles.outfitField}>
                <span className={detailStyles.outfitFieldValue}>{item.brand || "브랜드 미등록"}</span>
              </div>
              <div className={detailStyles.outfitField}>
                <span className={detailStyles.outfitFieldValue}>
          ...
          <div className={detailStyles.emptyText}>등록된 아이템이 없습니다.</div>
        )}
      </div>
