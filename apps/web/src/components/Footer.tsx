...
const Footer: React.FC = () => {
  ...
  return (
    // 🔴 1. 최상위에 흰색 배경 영역을 추가하여 회색 배경을 가립니다.
    <div className={styles.whiteBackground}>
      {/* 기존 푸터 컨테이너는 이 내부에 위치합니다. */}
      <div className={styles.footerContainer}>
        ...
        <div className={styles.navItem} onClick={() => navigate('/rankingZone')}>
          <div className={styles.iconBox}>
            <RankingIcon color={isActive('/rankingZone') ? '#FFFFFF' : '#666666'} />
          </div>
          <span className={`${styles.navText} ${isActive('/rankingZone') ? styles.activeText : ''}`}>랭킹</span>
        </div>

        {/* 중앙 플러스 버튼: 하얀 원형 */}
        <div className={styles.addBtnWrapper} onClick={() => navigate('/post/write')}>
        ...
        <div className={styles.navItem} onClick={() => navigate('/evaluationZone')}>
          ...
          <span className={`${styles.navText} ${isActive('/vote') ? styles.activeText : ''}`}>평가</span>
        </div>
      </div>
    </div>
  );
